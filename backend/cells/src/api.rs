//! Low-level cell construction contract shared by built-in and custom cells.
//!
...
/// Read-only process context supplied to custom and built-in cell generators.
///
/// Keeping the verification deck and construction parameters together prevents
/// a macro from accidentally drawing with values from a different PDK.
#[derive(Clone, Copy)]
...
impl<'a> CellContext<'a> {
    #[must_use]
    pub const fn new(deck: &'a Deck, pdk: &'a Pdk) -> Self {
        Self { deck, pdk }
    }

    #[must_use]
    pub const fn deck(self) -> &'a Deck {
        self.deck
    }

    #[must_use]
    pub const fn pdk(self) -> &'a Pdk {
        self.pdk
    }
}
...
impl<'a> CellBuilder<'a> {
    /// Build without construction parameters. Intended for geometry-only unit
    /// tests; [`Self::pdk`] returns an error in this mode.
    ...
    /// Build with the validated process context used by production flows and
    /// user-defined `substrate3` macros.
    #[must_use]
    pub fn with_context(context: CellContext<'a>, tier: MatchingTier, group_id: u32) -> Self {
        let mut builder = Self::new(context.deck, tier, group_id);
        builder.pdk = Some(context.pdk);
        builder
    }
    ...
    /// Coordinates and sizes snap to the manufacturing grid — odd finger_w
    /// divisions (415/2) were leaking off-grid coordinates into cells.
    pub fn rect(
        &mut self,
        layer: &str,
        x: i32,
        ...
        w: i32,
        h: i32,
    ) -> Result<PolyId, CellError> {
        let lid = self.resolve(layer)?;
        Ok(self.rect_id(lid, x, y, w, h))
    }

    /// Draw rectangle on a resolved `LayerId` (skip name lookup). Snaps.
    ...
    /// Draw polygon on named layer. Points snap to the manufacturing grid.
    pub fn polygon(&mut self, layer: &str, pts: &[(i32, i32)]) -> Result<PolyId, CellError> {
        let lid = self.resolve(layer)?;
        let snapped: Vec<(i32, i32)> = pts
            .iter()
            .map(|&(x, y)| (self.snap(x), self.snap(y)))
            .collect();
        Ok(self.store.add_polygon(lid, &snapped))
    }

    /// Declare a pin access point. Snaps like `rect` so pins stay coincident
    /// with the geometry they were computed from.
    pub fn pin(
        &mut self,
        name: &str,
        layer: &str,
        ...
        let lid = self.resolve(layer)?;
        self.pin_id(name, lid, x, y, w, h);
        Ok(())
    }

    /// Declare a pin on a resolved `LayerId`. Snaps.
    ...
    pub fn instance(
        ...
        let mut sub = self.context().map_or_else(
            || CellBuilder::new(self.deck, self.tier, self.group_id),
            |context| CellBuilder::with_context(context, self.tier, self.group_id),
        );
        ...
        for p in sub.pins {
            let (rx, ry, rw, rh) = orient.transform_rect(p.x - ox, p.y - oy, p.w, p.h, cw, ch);
            self.pins.push(PinAccess {
                name: p.name,
                layer: p.layer,
                x: rx + dx,
    ...
    pub fn tier(&self) -> MatchingTier {
        self.tier
    }

    /// Complete process context when the builder came from the backend flow.
    #[must_use]
    pub fn context(&self) -> Option<CellContext<'a>> {
        self.pdk.map(|pdk| CellContext::new(self.deck, pdk))
    }

    /// PDK-specific layer roles and construction dimensions.
    pub fn pdk(&self) -> Result<&'a Pdk, CellError> {
        self.pdk.ok_or(CellError::MissingAnalogParams)
    }
    ...
    pub fn resolve(&self, name: &str) -> Result<LayerId, CellError> {
        self.layers
            .id(name)
            .ok_or_else(|| CellError::UnknownLayer { name: name.into() })
    }
    ...
    pub fn store_mut(&mut self) -> &mut GeometryStore {
        &mut self.store
    }
...
pub use gdsverify::{run_drc, DrcReport, Violation};
pub use gdsverify::{run_lvs, DeviceFlavor, DeviceKind, LvsResult, RefDevice, RefNetlist};
...
mod tests {
    ...
    fn hierarchical_instance() {
        ...
        let mut b = CellBuilder::new(&deck, MatchingTier::None, 0);
        b.instance("i0", &sub_cell, 0, 0, Orientation::R0)
            .expect("i0");
        b.instance("i1", &sub_cell, 200, 0, Orientation::R0)
            .expect("i1");
        let out = b.finish();
